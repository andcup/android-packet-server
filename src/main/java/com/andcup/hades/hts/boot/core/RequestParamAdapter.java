 ...
 * Description:
 */
class RequestParamAdapter {

    public static List<Object> adapter(RequestInvoker invoker, Map<String, String> values){
        List<Object> listValue = new ArrayList<Object>();
        Parameter[] parameters = invoker.method.getParameters();
        for(int i=0; i< parameters.length; i++){
            Var var = parameters[i].getAnnotation(Var.class);
            fillParams(listValue, parameters[i].getType(), values.get(var.value()));
        }
        return listValue;
    }

    private static void fillParams(List<Object> list, Class<?> parameter, String value) {
